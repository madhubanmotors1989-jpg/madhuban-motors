...
export function Footer() {
  ...
  return (
    <footer className="bg-[#0A0A0A] border-t border-white/06">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-3">
              <a href={SITE.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl bg-white/05 border border-white/08 flex items-center justify-center text-white/40 hover:text-white hover:border-white/20 transition-all">
                <InstagramIcon className="w-4 h-4" />
              </a>
              <a href={waLink} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl bg-white/05 border border-white/08 flex items-center justify-center text-white/40 hover:text-white hover:border-white/20 transition-all">
                <MessageCircle className="w-4 h-4" />
              </a>
              <a href={`tel:${SITE.phone}`} className="w-9 h-9 rounded-xl bg-white/05 border border-white/08 flex items-center justify-center text-white/40 hover:text-white hover:border-white/20 transition-all">
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {SERVICES.slice(0, 6).map((s) => (
                <Link key={s.id} href="#services" className="text-white/30 text-sm hover:text-[#E31837] transition-colors">{s.title}</Link>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <Link key={l.label} href={l.href} className="text-white/30 text-sm hover:text-[#E31837] transition-colors flex items-center gap-2">
                  {l.label}
                  {l.label === "Online Shop" && (
                    <span className="text-[10px] text-[#E31837] border border-[#E31837]/30 px-1.5 py-0.5 rounded-full">Soon</span>
                  )}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-5">
              <div className="flex items-start gap-3">
                <MapPin className="w-4 h-4 text-[#E31837] mt-0.5 flex-shrink-0" />
                <p className="text-white/30 text-sm leading-relaxed">{SITE.address}</p>
              </div>
              <div className="flex items-start gap-3">
                <Phone className="w-4 h-4 text-[#E31837] mt-0.5 flex-shrink-0" />
                <div>
      ...
      <div className="border-t border-white/04 py-5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <p className="text-white/10 text-xs">MyTVS Authorized Partner · GST Registered · Ghaziabad, UP</p>
        </div>
      </div>
    </footer>
  );
